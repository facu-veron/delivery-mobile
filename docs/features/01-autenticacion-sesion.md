...
> Estado: **implementado**  
> Archivos clave: `src/infrastructure/auth/supertokens.config.ts`, `src/presentation/routes/auth.routes.ts`
...
## Login — respuesta de credenciales incorrectas

SuperTokens ahora devuelve **HTTP 401** cuando las credenciales son inválidas (antes siempre era 200).

```
POST /auth/signin
Body: { "formFields": [{"id":"email","value":"x"},{"id":"password","value":"y"}] }

→ 401  { "status": "WRONG_CREDENTIALS_ERROR" }
→ 200  { "status": "OK" }
```
...
Manejado por SuperTokens: `POST /auth/signout`
...
- Si el usuario cambia su nombre con `PATCH /api/me`, el token no se actualiza hasta que cierre y vuelva a abrir sesión. Usar `GET /api/me` para el nombre actualizado en esa sesión.
