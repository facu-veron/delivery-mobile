    ...
    <SafeAreaView className="flex-1 bg-background dark:bg-background-dark">
      <FlatList<Pedido>
          ...
          <View className="px-4 pt-4 pb-3">
            ...
            {pedidos && pedidos.length > 0 ? (
              <Text className="text-sm text-muted-foreground dark:text-muted-dark-foreground mt-0.5">
                {pedidos.length} entrega{pedidos.length !== 1 ? 's' : ''} completada{pedidos.length !== 1 ? 's' : ''}
              </Text>
            ) : null}
          </View>
        }
        ...
        ListEmptyComponent={
          <EmptyState
            icon={History}
            title="Todavía no tenés entregas"
            description="Acá vas a ver el historial de tus entregas completadas y tu ganancia."
          />
        }
      />
    </SafeAreaView>
